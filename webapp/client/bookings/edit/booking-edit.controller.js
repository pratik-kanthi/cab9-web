(function (angular) {
    ...
    function BookingCreateController($scope, $http, $config, $state, $timeout, Auth, Google, $tenantConfig, Model, rClients, rPassengers, rVehicleTypes, rVehicleClasses, rDrivers, rVehicles, Localisation, $filter, rCurrencies, rTaxes, $window) {
        ...
        function startNew() {
            ...
            $scope.selectedBooking.ClientId = $scope.CLIENTID.toLowerCase();
        }
    ...
    BookingEditController.$inject = ['$scope', '$q', '$http', '$config', '$timeout', 'Google', '$tenantConfig', 'Model', 'rBooking', 'rClients', 'rVehicleTypes', 'rVehicleClasses', 'rDrivers', 'rVehicles', 'rPassengers', 'Localisation', '$filter', 'rTaxes', '$window', 'Auth', 'Notification', '$modal'];

    function BookingEditController($scope, $q, $http, $config, $timeout, Google, $tenantConfig, Model, rBooking, rClients, rVehicleTypes, rVehicleClasses, rDrivers, rVehicles, rPassengers, Localisation, $filter, rTaxes, $window, Auth, Notification, $modal) {
        $scope.viewMode = "VIEW";
        var currentQuote = null;
        $scope.showTraffic = false;
        $scope.setupComplete = false;
