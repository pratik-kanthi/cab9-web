(function (angular) {
    ...
    BookingCreateController.$inject = ['$scope', '$http', '$config', '$state', '$timeout', 'Auth', 'Google', '$tenantConfig', 'Model', 'rClients', 'rPassengers', 'rVehicleTypes', 'rVehicleClasses', 'rDrivers', 'rVehicles', 'Localisation', '$filter', 'rCurrencies', 'rTaxes', '$window'];

    function BookingCreateController($scope, $http, $config, $state, $timeout, Auth, Google, $tenantConfig, Model, rClients, rPassengers, rVehicleTypes, rVehicleClasses, rDrivers, rVehicles, Localisation, $filter, rCurrencies, rTaxes, $window) {
        ...
        $scope.directions = {
            ...
            setDirections: function (directions) {
                ...
                if (this.currentRoute && this.currentRoute.legs && this.currentRoute.legs[0]) {
                    ...
                    $scope.selectedBooking.EstimatedDistance = this.currentRoute.legs[0].distance.value * 0.000621371;
                }
            },
            options: {
        ...
        $scope.$watch('selectedBooking.BookingStops', function () {
            ...
            if (canQuote) {
                ...
                $scope.selectedBooking.DriverExclusions = 0;
                $scope.selectedBooking.DriverCost = 0;
        ...
        $scope.$watch("selectedBooking.VehicleTypeId", function (newValue) {
            Model.Vehicle
                ...
                .parseAs(function (item) {
                    ...
                    this.ImageUrl = formatUrl(item.ImageUrl, item.Registration);
                }).execute()
                .then(function (data) {
                    $scope.vehicles = data;
                });
        ...
        function getQuote() {
            console.log($scope.selectedBooking.BookingStops);
        ...
        $scope.$watch('selectedBooking.CurrencyId', function () {
            ...
            $scope.selectedBooking.ActualCost = Number((($scope.ActualCost || 0) * found.CurrentRate).toFixed(2));
            $scope.selectedBooking.EstimatedCost = Number((($scope.EstimatedCost || 0) * found.CurrentRate).toFixed(2));

            if ($scope.selectedBooking.DriverCost)
                $scope.selectedBooking.DriverCost =  Number((($scope.DriverCost || $scope.selectedBooking.DriverCost) * found.CurrentRate).toFixed(2));
            if ($scope.selectedBooking.DriverExclusions)
                $scope.selectedBooking.DriverExclusions =  Number((($scope.DriverExclusions || $scope.selectedBooking.DriverExclusions) * found.CurrentRate).toFixed(2));
            if ($scope.selectedBooking.InvoiceCost)
                $scope.selectedBooking.InvoiceCost =  Number((($scope.InvoiceCost || $scope.selectedBooking.InvoiceCost) * found.CurrentRate).toFixed(2));
            if ($scope.discount.type == "amount") {
                $scope.selectedBooking.Discount = Number((($scope.Discount || $scope.selectedBooking.Discount) * found.CurrentRate).toFixed(2));
            }
        ...
        function book(booking) {
            ...
            var time = new moment($scope.selectedBooking.Time).tz(Localisation.timeZone().getTimeZone());
    ...
    BookingEditController.$inject = ['$scope', '$q', '$http', '$config', '$filter', '$timeout', 'Google', '$tenantConfig', 'Model', 'rBooking', 'rClients', 'rVehicleTypes', 'rVehicleClasses', 'rDrivers', 'rVehicles', 'rPassengers', 'Localisation', '$filter', 'rTaxes', '$window', 'Auth', 'Notification', '$modal'];

    function BookingEditController($scope, $q, $http, $config, $filter, $timeout, Google, $tenantConfig, Model, rBooking, rClients, rVehicleTypes, rVehicleClasses, rDrivers, rVehicles, rPassengers, Localisation, $filter, rTaxes, $window, Auth, Notification, $modal) {
        $scope.viewMode = "VIEW";
        var currentQuote = null;
        $scope.showTraffic = false;
        $scope.setupComplete = false;
        ...
        var dt = new moment(rBooking.BookedDateTime);
        dt.tz(Localisation.timeZone().getTimeZone());
        $scope.selectedBooking.Time = dt.startOf('minute').toDate();
        ...
        if ($scope.selectedBooking.ActualCost) $scope.selectedBooking.ActualCost = Number($scope.selectedBooking.ActualCost.toFixed(2));
        if ($scope.selectedBooking.DriverCost) $scope.selectedBooking.DriverCost = Number($scope.selectedBooking.DriverCost.toFixed(2));
        if ($scope.selectedBooking.EstimatedCost) $scope.selectedBooking.EstimatedCost = Number($scope.selectedBooking.EstimatedCost.toFixed(2));
        if ($scope.selectedBooking.InvoiceCost) $scope.selectedBooking.InvoiceCost = Number($scope.selectedBooking.InvoiceCost.toFixed(2));
        if ($scope.selectedBooking.Discount) $scope.selectedBooking.Discount = Number($scope.selectedBooking.Discount.toFixed(2));
        ...
        function save() {
            ...
            var time = new moment($scope.selectedBooking.Time).tz(Localisation.timeZone().getTimeZone());
            var bdt = new moment($scope.selectedBooking.BookedDateTime).format("YYYY-MM-DD") + "T" + time.format("HH:mm") + ":00" + moment.tz(Localisation.timeZone().getTimeZone()).format('Z');
        ...
        function getQuote() {
            $http.post($config.API_ENDPOINT + 'api/quote', $scope.selectedBooking)
                .success(function (result) {
                    ...
                    $scope.EstimatedCost = result.FinalCost;
                    $scope.selectedBooking.EstimatedCost = Number(($scope.EstimatedCost * $scope.selectedBooking.CurrencyRate).toFixed(2));
                    $scope.selectedBooking.EstimatedMins = result.EstimatedMins;
                }).error(function (error) {
                    console.log(error);
                });
