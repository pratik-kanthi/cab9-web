(function (angular) {
    ...
    module.controller('SettingsDriverPaymentFixedController', SettingsDriverPaymentFixedController);
    SettingsDriverPaymentFixedController.$inject = ['$scope', '$UI', '$q', '$state', '$stateParams', 'Model', 'rFixedCount', 'rFixeds', 'rVehicleTypes', 'rZones', '$http', '$config', '$modal'];

    function SettingsDriverPaymentFixedController($scope, $UI, $q, $state, $stateParams, Model, rFixedCount, rFixeds, rVehicleTypes, rZones, $http, $config, $modal) {
        $scope.paging = {
            currentPage: 1,
            resultsPerPage: 10,
            totalResults: null,
        ...
        for (i = 0; i < $scope.fixeds.length; i++) {
            if ($scope.fixeds[i].Amount != undefined || $scope.fixeds[i].Amount != null)
                $scope.fixeds[i].Amount = $scope.fixeds[i].Amount.toFixed(2);
        ...
        function startNew() {
            $scope.addNew = true;
            $scope.newfixed = new Model.DriverPaymentModelFixed();
            $scope.newfixed.DriverPaymentModelId = $scope.item.Id;
        }
        ...
        function saveNew(newfixed) {
            ...
            function saveDetails(newfixed) {
                newfixed.Amount = parseFloat(newfixed.Amount)
                $http.post($config.API_ENDPOINT + 'api/DriverPayments/addNewDriverFixed', newfixed)
                    .then(function (response) {
                        ...
                        angular.forEach($scope.fixeds, function (f) {
                            var vts = f.VehicleTypes.map(function (v) { return new Model.VehicleTypeInDriverPaymentFixed(v); });
        ...
        function startImport() {
            $modal.open({
                templateUrl: '/webapp/management/settings/driverpayments/fixed/import.modal.html',
                controller: ['$scope', '$config', '$q', 'Auth', '$http', '$state', '$modalInstance', function ($scope, $config, $q, Auth, $http, $state, $modalInstance) {
                    ...
                    function validateFile() {
                        ...
                        function onSuccess(data) {
                            ...
                            } else if ($scope.import.hasErrors) {
                                swal({
                                    type: 'error',
                                    ...
                                    $modalInstance.close();
                                    $state.go('root.settings.import.driver-prices', {
                                        progress: $scope.import
                                    });
                                });
                            } else if ($scope.import.hasWarnings) {
                                swal({
                                    type: 'error',
                                    title: 'Warnings validating file',
                                    text: 'Would you like to view the warnings?',
                                    showCancelButton: true,
                                    confirmButtonText: 'Yes, show me the warnings.',
                                    cancelButtonText: 'No, continue with upload.'
                                }, function (result) {
                                    if (result) {
                                        $modalInstance.close();
                                        $state.go('root.settings.import.driver-prices', {
                    ...
                    function importFile() {
                        ...
                        return $http.post($config.API_ENDPOINT + 'api/Import/DriverFixed/Confirm', {
        ...
        function deleteSelected() {
            var selected = $scope.fixeds.filter(function (fixed) {
            ...
            if (selected.length > 0) {
                swal({
                    ...
                    $http.post($config.API_ENDPOINT + 'api/DriverPayments/deleteSelectedDriverFixed', selected).then(function () {
        ...
        function filter() {
            var query = Model.DriverPaymentModelFixed.query()
                .select('Id')
                .parseAs(function (data) {
                    this.Id = data.Id;
                })
                .filter('DriverPaymentModelId', 'eq', "guid'" + $stateParams.Id + "'");
        ...
        function setPage(page) {
            ...
            var queryString = 'api/DriverPayments/FetchDriverFixedDetails?driverPaymentModelId=' + $stateParams.Id + '&count=200&pageNum=' + page;
            if ($scope.filters.fromZone && $scope.filters.fromZone != "")
        ...
        function getVehicleTypeCost(fixed, vt) {
            if (fixed != undefined && fixed.VehicleTypes != null) {
                ...
                if (typePricing) {
                    return typePricing.FixedPayment.toFixed(2);;
                } else {
                    return null;
                }
            }
        ...
        function setVehicleTypeCost(fixed, vt, value) {
            if (fixed != undefined && fixed.VehicleTypes != null && fixed.Id != undefined) {
                ...
                if (typePricing) {
                    typePricing.FixedPayment = parseFloat(value);
                } else if (value) {
                    var n = new Model.VehicleTypeInDriverPaymentFixed();
                    n.VehicleTypeId = vt.Id;
                    n.DriverPaymentModelId = fixed.Id;
                    n.FixedPayment = parseFloat(value);
