(function(angular) {
    ...
    DispatchNewBookingController.$inject = ['$scope', '$UI', '$modal', '$config', '$q', 'Model', '$stateParams', '$rootScope', 'Localisation', '$utilities', '$http', '$timeout', '$filter', 'rCompany', 'rTags'];

    function DispatchNewBookingController($scope, $UI, $modal, $config, $q, Model, $stateParams, $rootScope, Localisation, $utilities, $http, $timeout, $filter, rCompany, rTags) {
        ...
        //$scope._dbooking = $scope.dispatchObj.newBooking;
        $scope.paxMode = {
            value: false
        };
        ...
        function setupData() {
            ...
            var _clientP = Model.Client.query().where('Active eq true')
                ...
                .parseAs(function(item) {
                    ...
                    this.Tags = item.Tags;
                    this.ClientReferences = item.ClientReferences.filter(function(r) {
                        return r.Active;
                    }).map(function(cr) {
                        return {
                            Id: cr.Id,
        ...
        function prefillInfoSelected() {
            ...
            var selected = JSON.parse($scope.dispatchObj.selectedCaller);
        ...
        function handlePhoneCallNewBooking() {
            if ($scope.dispatchObj.phoneCall) {
                $scope._dbooking.Calls = [];
                $scope._dbooking.Calls.push($scope.dispatchObj.phoneCall)
            }
        }
            ...
            if (item.custom) {
                ...
                swal({
                    ...
                    if (isConfirm) {
                        ...
                        console.log('refSelected - $scope.$apply');
                        $scope.$apply();
                    }
                });
            }
        ...
        function addTag(item, $select) {
            ...
            if(!tag) {
                ...
                if ($select) $select.selected = null;
            }
        }
        ...
        $scope.$watch("_dbooking.LeadPassengerId", function(newvalue) {
            ...
            if (newvalue) {
                ...
                b.PassengerNotificationPhone = $scope.selectedPassenger.Mobile ? $scope.selectedPassenger.Mobile : $scope.selectedPassenger.Phone;
        ...
        $scope.$watch("_dbooking.ClientId", function(newvalue) {
                    ...
                    if (found.AccountPassword) {
                        ...
                        swal({
                            ...
                            if (inputValue == found.AccountPassword) {
                                $scope._dbooking.ClientId = newvalue;
                    ...
                    function setupClient() {
                        ...
                        if (found) {
                            ...
                            if (found.DefaultPaymentType) {
                                ...
                                //if (found.DefaultPaymentType == 'Card') {
                                //    $scope.showPaymentCards = true;
                                //}
                            }
        ...
        function save(repeat) {
            console.log($scope._dbooking);
            $("#bookingsave").prop("disabled", true);
            $("#bookingrepeat").prop("disabled", true);

            //check if this is a repeat booking and check the validity
            var scheduled = false;
            ...
            $q.all(promises).then(function(result) {
                ...
                $("#bookingsave").prop("disabled", false);
                $("#bookingrepeat").prop("disabled", false);
                $scope.dispatchObj.phoneCall = null;
                if (!repeat) {
                    cancel();
                    $scope.dataReady = true;
        ...
        $scope.$watch("_dbooking.WaitAndReturn", function(newvalue) {
            if (newvalue === false) {
                ...
                var stop = new Model.BookingStop(angular.copy($scope._dbooking.BookingStops[0]));
                stop.id = idCounter++;
                stop.StopOrder = $scope._dbooking.BookingStops.length - 1;
                ...
                .success(function(result) {
                    ...
                    if (result.Directions && result.Directions.EncodedRoute) {
                        ...
                        if (directionsPolyline) {
                            ...
                            directionsPolyline.setMap($scope.dispatchObj.map.mapObject);
                        } else {
